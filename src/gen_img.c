		...
		for( _x = 0; _x < _width; ++_x )
		{
			...
			switch( imageFuncNum )
			{
				...
				default:
					...
					*/
					// this code will run if you pass the -c flag to the built binary
					/*
						================================================================================================================

						Here's a short handy list of functions you have access to:

						lut_sin(long a) -> sin
						lut_cos(angle) -> cosine
						lut_angle(long dx, long dy) -> measure angle between two points with the center as a corner
						lut_dist(long x, long y) -> distance in pixels between two points
						xor(a, b) -> exclusive or function - this means for two numbers it flips the bit only if different:	000011010101
																															000101101000
																															000110101101
						mod(a, b) -> two floating point numbers (ex. 1.2 / 1.4) returns the remainder after division
						RANDOM(r) -> a random integer between 1 and r: RANDOM(10) would produce a number 1-10

						================================================================================================================

						and some other handy variables:

						x, y -> current index for pixel value
						xcenter -> this is the middle of the screen on the X axis
						ycenter -> "" but for y axis
						width, height -> is what you think it is
						dx -> x - xcenter;
						dy -> y - ycenter;
						dist = lut_dist(dx, dy);
						angle = lut_angle(dx, dy);
						...
						color -> THIS IS THE VALUE YOU WANT TO SET TO SEE A CHANGE ON THE SCREEN. WHATEVER CODE YOU WRITE WILL LOOK LIKE

						color = lut_sin(lut_dist(dx, dy)) + (dx * 1.618) * (dy / (144 / 89)); <- this is one that I made, looks pretty neat

					*/

					/*
					One final note: this variable is named 'color' but this doesn't actually set the RGB value, here's what this actually becomes:
					...
					(buf_graf + (pitch * _y) + _x) = (uint8_t) _color; <-- this ends up actually setting the color, but values are arranged like this:
					// ...[RGB][RGB][RGB][RGB][RGB][RGB][RGB][RGB][RGB][RGB][RGB][RGB][RGB][RGB]...
					================================================================================================================

					so 'color' the variable just determines where in the current pallete this value falls
					don't really fully understand how it works, but you can just accept that modifying this value affects the "shape" of the generated image.
					If you want to mess with the colors, see the other 'ATTENTION CHRIS' note in palinit.c
					*
					*/
